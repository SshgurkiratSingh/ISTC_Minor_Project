    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/frontend//getSpecificData/room${roomId}`
        );
        if (!response.ok) {
        ...
        <div className="flex flex-col p-2">
          <div className="flex xl:flex-row flex-col m-2 p-2 mb-8">
            {Array.from({ length: noOfLights }).map((_, index) => (
              <ToggleButton
                key={index}
                topic={`IoT/room${roomId}/light${index + 1}`}
                subTitle={`Light ${index + 1}`}
                value={
                  ServerData?.find(
                    (item) =>
                      item.topic === `IoT/room${roomId}/light${index + 1}`
                  )?.value === "1"
                }
              />
            ))}
          ...
          <div className="mb-14">
            {Array.from({ length: noOfFans }).map((_, index) => (
              <FanSpeedSelector
                key={index}
                topic={`IoT/room${roomId}/fan${index + 1}`}
                subTitle={`Fan ${index + 1}`}
                value={Number(
                  ServerData?.find(
                    (item) => item.topic === `IoT/room${roomId}/fan${index + 1}`
                  )?.value
                )}
              />
            ))}
